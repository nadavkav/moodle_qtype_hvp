...
class content_user_data {
    ...
    public static function handle_ajax() {
        ...
        if ($contentid === null || $dataid === null || $subcontentid === null) {
            // Missing parameters.
            \H5PCore::ajaxError(get_string('missingparameters', 'qtype_hvp'));
            return;
        }
    ...
    private static function store_data($contentid, $subcontentid, $dataid, $data, $preload, $invalidate) {
        ...
        if (!has_capability('qtype/hvp:savecontentuserdata', $context)) {
            \H5PCore::ajaxError(get_string('nopermissiontosavecontentuserdata', 'qtype_hvp'));
    ...
    public static function get_user_data($contentid, $subcontentid, $dataid) {
        ...
        $result = $DB->get_record('qtype_hvp_content_user_data', array(
                'user_id' => $USER->id,
                'hvp_id' => $contentid,
    ...
    public static function save_user_data($contentid, $subcontentid, $dataid, $preload, $invalidate, $data) {
        ...
        if ($update === false) {
            // Insert new data.
            $DB->insert_record('qtype_hvp_content_user_data', $newdata);
        } else {
            // Get old data id.
            $newdata->id = $update->id;

            // Update old data.
            $DB->update_record('qtype_hvp_content_user_data', $newdata);
        }
    }
    ...
    public static function delete_user_data($contentid, $subcontentid, $dataid) {
        ...
        $DB->delete_records('qtype_hvp_content_user_data', array(
            'user_id' => $USER->id,
            'hvp_id' => $contentid,
            'sub_content_id' => $subcontentid,
            'data_id' => $dataid
    ...
    public static function load_pre_loaded_user_data($contentid) {
        ...
        $results = $DB->get_records('qtype_hvp_content_user_data', array(
            'user_id' => $USER->id,
            'hvp_id' => $contentid,
            'sub_content_id' => 0,
            'preloaded' => 1
